...
def build_idf(bodies, stances, pos_tags=None):
    ...
    tokenized_corpus = [helpers.get_clean_tokens(x) for x in corpus]
    idf = feature_engineering.build_idf_tokens(tokenized_corpus, pos_tags)
...
def process_body(body, idf=None):
    ...
    clean_sentences = [helpers.get_clean_tokens(s) for s in sentences]
    ...
    first_sentence_data = feature_engineering.process_sentence(first_sentence)
    ...
    if idf == None:
        ...
        sentence_importance = [(s, feature_engineering.score_sentence(s, word_count))
                               for s in clean_sentences]
        most_significant_sentence, sentence_score = list(
            sorted(sentence_importance, key=lambda x: x[1]))[-1]
        most_significant_sentence_data = feature_engineering.process_sentence(
        ...
        sentence_importance = [
            (s, feature_engineering.score_sentence(s, word_count, idf)) for s in clean_sentences]
        most_significant_sentence, sentence_score = list(
            sorted(sentence_importance, key=lambda x: x[1]))[-1]
        most_significant_sentence_data = feature_engineering.process_sentence(
            ' '.join(most_significant_sentence))
    ...
    vader_sentiment = feature_engineering.sentiment_multi(body)
...
# word embeddings helpers


def extract_word_embeddings(filename):
    return word_embeddings.extract_word_embeddings(filename)


def get_glove_dict(name):
    return word_embeddings.get_glove_dict(name)
